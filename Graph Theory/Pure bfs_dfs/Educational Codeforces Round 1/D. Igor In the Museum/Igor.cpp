...
// solve time ~ 15mins, fastest 1700 so far?
// I dont think I need to write an explanation for this, its quite self-explanatory.
    ...
    for (int i=0; i<n; i++){
        for (int j=0; j<m; j++){
            if (grid[i][j] == '.'){
                ...
                for (auto [x, y] : component) ans[x][y] = count;
            }
        }
    }
