...
// solve time ~ 1.2hrs
// very similar problem to minimum maximum distance (1700 rated)
...
void solve() {
    ...
    vi marked(m); // affected towns
    for (int& x: marked) {cin >> x; x--;}
    ...
    for (int e : marked){
        if (dist2[e] > 2*d) {cout << 0 << '\n'; return;}
        if (dist2[e] > dist2[farthestNode2]) farthestNode2 = e;
    }

    // 3rd DFS to get distance from farthestNode2
    vi dist3(n, -1);
    dfs(farthestNode2, -1, adjList, dist3, 0);

    int ans = 0;
    for (int node=0; node<n; node++){
        if (dist3[node] <= d && dist2[node] <= d) ans++;
    }

    cout << ans << '\n';
    // cout << farthestNode1 << endl;
    // cout << farthestNode2 << endl;
    // printVector(dist2);
}
