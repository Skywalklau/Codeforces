...
void solve() {
    ...
    for (ll i=1; i<=n; i++){
        ...
        // handle not include current element case
        for (ll x = 0; x < 3; x++){
            for (ll y = 0; y < 3; y++) {
        ...
        for (ll y=0; y<3; y++){
            // handle include current element case
    ...
    // we cannot do sum(dp[n][0][0]+dp[n][1][0]+dp[n][0][1]+dp[n][1][1])
    // because the length must be >= 3. So, we sum at every >= 3 in the loop.
    cout << ans << '\n';
