...
void solve() {
    ...
    vll dp((1<<n), MAXL);
    dp[0] = 0; // 0 extensions for 0 assignments

    vll sumTime((1<<n), 0);
    for (int mask=1; mask < (1<<n); mask++){
        for (int i=0; i<n; i++){
            if (mask & (1 << i)) sumTime[mask] += c[i];
        }
    }

    for (int mask=0; mask<(1<<n); mask++){
        if (dp[mask] >= MAXL) continue;
        ll time = sumTime[mask];
        for (int i=0; i<n; i++){
            if (!(mask & (1<<i))){
                ll finish = time + c[i];
