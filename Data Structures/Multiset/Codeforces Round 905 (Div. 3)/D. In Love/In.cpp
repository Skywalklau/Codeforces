...
// solve time ~ 10 mins
// nice problem with multiset
// initially I thougth fenwick, but thats a bit overkill.
// We can use mset, then just store the l and r respectively in 2 mset.
// then, we can just check the largest l with the smallest r,
// if the largest l > smallest r, then this means there exists an interval
// that does not overlap, so return YES
// else NO
// and thats the solution.
// Key observation:
// For 2 intervals to not intersect:
// max l > min r (very common idea in interval intersection problems)
