...
// solve time ~ 40 mins
// Nice problem, implementation was a bit tricky.
// But yea the idea is very simple, just sort + greedy.
// The hard part is simulating the actions correctly.
// We need to correctly maintain our current ammo and the round we are on.
// Look at the code for clarity.
...
void solve() {
    ...
    // we obviously wanna kill monsters closer to us first
    ...
    for (int i=0; i<n; i++){
        ...
        if (ammo >= hp){
            ammo -= hp;
            if (ammo == 0) {ammo = k; round++;}
        } 
        else{
            hp -= ammo;
            // cout << "hp: " << hp << endl;
            round += ((hp+k-1) / k);
            ammo = (hp % k == 0 ? 0 : k - (hp % k));
            // cout << i << endl;
            // cout << round << " " << ammo << endl;
            if (pos - round <= 0){
                cout << "NO" << '\n';
                return;
            }
