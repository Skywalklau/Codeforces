...
// Its just so obvious that its bs for ans.
// the check we can do in O(n).
// We do it greedily, as soon as we have all values from 0 to mid-1.
// then we can ans++ to indicate we take on subarray here then continue.
// In the end, if we have ans >= k then we know this mid is valid, so lets try
// a bigger mex via mid.
// and thats it.

bool check(vi& a, int k, int n, int mid){
    vi cnt(mid+1);
    int m = 0, p = 1, ans = 0;
    for (int i=0; i<n; i++){
        if (a[i] < mid && cnt[a[i]] < p){
            m++;
            cnt[a[i]]++;
            if (m == mid) {ans++; p++; m=0;}
        }
