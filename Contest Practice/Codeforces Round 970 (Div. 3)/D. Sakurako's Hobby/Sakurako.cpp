...
// solve time ~ 34 mins
// Yea initially I thought the problem seemed interesting, but turns
// out its just a plain old dfs on cycles. 
// At first glance this was weird, then I tried sketching it out then
// you will realize that we just find the number of black nodes in 
// a cycle, and then adding the number of black nodes in the cycle
// to each node in the cycle.
// And thats literally the solution. Trivial.

vi temp;
int ans = 0;
void dfs(int curNode, vb& visited, vi& a, str& s){
    if (visited[curNode]) return;
    visited[curNode] = true;
    if (s[curNode-1] == '0') ans++;
    temp.pb(curNode);
    dfs(a[curNode], visited, a, s);
}   
...
void solve() {
    ...
    for (int i=1; i<=n; i++){
        if (!visited[i]){
            ...
            // printVector(temp);
            // cout << ans << '\n';
            for (int node : temp) result[node] = ans;
        }
    }
