...
// solve time 32 mins
// Well, not much explanation needed for this one.
// Its just implementation innit?
// The only notable thing is that we need to take double of the resultant
// string because its circular.

int search(vstr& a, int r1, int r2, int c1, int c2){
    str result = "";
    
    for (int i=c1; i<=c2; i++) result += a[r1][i];
    for (int j=r1+1; j<=r2; j++) result += a[j][c2];
    for (int i=c2-1; i>=c1; i--) result += a[r2][i];
    for (int j=r2-1; j>=r1+1; j--) result += a[j][c1];

    int n = sz(result);
    result += result;
    int ans = 0;
    for (int i=0; i<n; i++){
        if (result.substr(i, 4) == "1543") ans++;
    }
...
void solve() {
    ...
    while (startr < endr && startc < endc){
        res += search(a, startr, endr, startc, endc);
        startr++;
        endr--;
        startc++;
        endc--;
