...
void dfs(int curNode, int parentNode, vvi& adjList, vi& depth, int h){
    depth[curNode] = h;
    if (sz(adjList[curNode]) == 1 && node1 == -1) node1 = curNode; 
    else if (sz(adjList[curNode]) == 1 && node2 == -1) node2 = curNode;
...
void solve() {
    ...
    if (sz(adjList[start]) == 1){
        int ans = *mxe(all(depth));
        cout << (ans % 2 == 1 ? "Ron" : "Hermione") << '\n';
    }
    else{
        int ans1 = depth[node1];
        int ans2 = depth[node2];
        if (ans1 % 2 == 1 || ans2 % 2 == 1) cout << "Ron" << '\n';
        else cout << "Hermione" << '\n';
    }
