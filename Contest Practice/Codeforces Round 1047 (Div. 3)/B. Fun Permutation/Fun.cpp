...
// Wow this was a really nice problem.
// The key takeaway: When adding 2 permutations at some index, we can always add them
// to be n+1.
// ie. p[i] + q[i] == n+1 for all i from 1 to n. This is always possible.
// Hence, the gcd will be with itself, which since n>=2, then n+1>=3. So all between
// adjacent elements will be >=3.
