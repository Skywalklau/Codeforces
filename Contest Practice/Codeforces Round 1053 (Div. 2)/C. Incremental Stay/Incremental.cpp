...
// solve time ~ 50 mins
// Honestly, this was a really standard problem. B was harder than this.
// This is just prefix + suffix sums.
// 
...
void solve() {
    ...
    vll suff(2*n+2), pref(2*n+1);

    for (int i=1; i<=2*n; i++) pref[i] = pref[i-1] + a[i]; 
    for (int i=2*n; i>=1; i--) suff[i] = suff[i+1] + a[i];
