...
void solve() {
    ...
    for (int i=0; i<P; i++){
        ...
        cin >> k;
        vpii path(k);
        for (int j=0; j<k; j++) path[j] = readCoord();
        vpii cyc;
        for (int j=0; j<k; j++) cyc.pb(path[j]);
        for (int j=k-2; j>=0; j--) cyc.pb(path[j]);
        patrol[i] = cyc;
        periods.pb(sz(cyc));
    }
    ...
    int cycle = 1;
    for (int p: periods) cycle = lcm(cycle, p);

    vvvb danger(cycle, vvb(R, vb(C, false)));    
    for (int t=0; t<cycle; t++){
        vvb seen(R, vb(C, false));
        for (int i=0; i<P; i++){
            auto [mr, mc] = patrol[i][t % sz(patrol[i])];
            seen[mr][mc] = true;
    ...
    vvvi dist(cycle, vvi(R, vi(C, -1)));
    dist[0][start.F][start.S] = 0;
    queue<array<int, 3>> q;
    ...
    while (!q.empty()){
        ...
        for (int d=0; d<5; d++){
            ...
            if (dist[nt][nr][nc] != -1) continue;

            if (nr == end.F && nc == end.S){
                cout << dist[t][r][c] + 1 << '\n';
                return;
            }

            if (danger[nt][nr][nc]) continue;
            dist[nt][nr][nc] = dist[t][r][c] + 1;
            q.push({nt, nr, nc});
        }
    }
