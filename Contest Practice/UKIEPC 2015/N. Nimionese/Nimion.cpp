...
void solve() {
    ...
    set<ch> hard = {'b', 'c', 'd', 'g', 'k', 'n', 'p', 't'};
    map<ch, str> convertLast = {
        {'b', "ah"}, {'c', "ah"}, {'d', "ah"}, {'g', "ah"},
        {'k', "oh"}, {'n', "oh"}, {'p', "oh"}, {'t', "uh"}
    };
    ...
    for (int i=0; i<n; i++){
        ...
        ch origFirst = tolower(a[i][0]);
        ch nearest = 'b';
        int minDist = abs(origFirst - 'b');
        for (ch h : hard){
            int dist = abs(origFirst - h);
            if (dist < minDist || (dist == minDist && h < nearest)){
                minDist = dist;
                nearest = h;
            }
        }

        if (isupper(a[i][0])) result[i].pb(toupper(nearest));
        else result[i].pb(nearest);
        ...
        for (int j=1; j<m; j++){
            ...
            ch c = a[i][j];
            if (!firstSyllable && hard.count(tolower(c))) c = nearest;
            result[i].pb(c);
        }

        ch last = result[i].back();
        if (hard.count(tolower(last))){
            result[i] += convertLast[tolower(last)];
        }
    }
