...
// solve time ~ 30 mins
// I think this is a pretty straightforward constructive
// Just cheese it.
// Put all U initially.
// then the moment we reach k, then we stop and start making a cycle.
// We can cheese this by just sending all to the bottom right corner with the cell
// above it.
// Note that if k == n*n-1 then its impossible (straightforward to see why).
// Otherwise, k can be within the last row, in this case we change slightly
// by setting a[n-1][0] = 'R', then the rest of the remaining n*n-k-1 cells
// as 'L' to make a cycle.
...
void solve() {
    ...
    for (int i=0; i<n; i++){
        ...
        if (cnt < k){
            for (int j=0; j<n; j++){
                cnt++;
                if (cnt == k) {start = j+1; break;}
            }
        }
        if (cnt == k){
            for (int j=start; j<n; j++){
                if (j == n-1 && i == n-1) a[i][j] = 'U';
                else if (j == n-1) a[i][j] = 'D'; 
                else a[i][j] = 'R'; 
            } 
        }
    }
