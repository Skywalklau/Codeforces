...
// solve time ~ 10 mins.
// mex can be at most 3 since mex(0,1,2) = 3.
// Case 1: mex == 0
// then 0 = max(a1,a2,3) - min(a1,a2,a3)
// => min = max => all values of the array are equal.
// But, they cannot be equal to 0 because otherwise mex == 1.
// Case 2: 1 <= mex <= 3
// then min(a1,a2,a3) = 0.
// So, mex(a1,a2,a3) = max(a1,a2,a3).
// but mex is never among a1,a2 and a3. But, max is.
// So, contradiction.
// Hence impossible to form.
// So, the array must all be equal values.
...
void solve() {
    ...
    s.extract(-1);
    cout << ((sz(s) == 1 && *s.begin() != 0) || sz(s) == 0 ? "YES" : "NO") << '\n';
