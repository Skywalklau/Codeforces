...
// solve time ~ 45 mins
// Nice problem. Did it with Kadane + prefix sums + observation of game theory
...
void solve() {
    ...
    // printVector(pref);
    // printVector(suff);
    vll prefMax(n+1), suffMax(n+1);
    prefMax[n] = pref[n];
    suffMax[1] = suff[1];
    for (int i=n-1; i>=1; i--) prefMax[i] = max(pref[i], prefMax[i+1]);
    for (int i=2; i<=n-1; i++) suffMax[i] = max(suff[i], suffMax[i-1]);
    // printVector(prefMax);
    // printVector(suffMax);

    ll ans = -MAXL;
    for (int i=1; i<=n; i++){
        ll l = (i >= 2 ? suffMax[i-1] - suff[i] : 0);
        ll r = (i <= n-1 ? prefMax[i+1] - pref[i] : 0);
        ans = max(ans, a[i]+b[i] + max(0ll, l) + max(0ll, r));
    }
