...
// solve time ~ 57 mins
// Implementation problem with 2 pointers + greedy. Nice one.
    ...
    for (int i=1; i<n; i++){
        if (a[0][i] == a[1][i-1]){
            ...
            for (int j=i+1; j<n; j++){
                ...
                else if (a[0][j] == '0'){ // a[0][i] = 0, a[1][i-1] = 1.
                    result += '0';
                    cnt = 1;
                    i = j;
                    break;
                }
                else{ // a[0][i] = 1, a[1][i-1] = 0.
                    for (int p=j-1; p<n; p++) result += a[1][p];
                    cout << result << '\n';
                    cout << cnt << '\n';
                    found = true;
                    return;
        ...
        else if (a[0][i] == '0'){ // a[0][i] = 0, a[1][i-1] = 1.
            result += '0';
            cnt = 1;
        }
        else{ // a[0][i] = 1, a[1][i-1] = 0.
            for (int j=i-1; j<n; j++) result += a[1][j];
            cout << result << '\n';
            cout << cnt << '\n';
            found = true;
            return;
