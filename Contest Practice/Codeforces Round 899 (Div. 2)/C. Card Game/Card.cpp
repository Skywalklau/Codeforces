...
// solve time ~ can't solve. Yea this was a nice problem.
// As for the explanation, the editorial is perfect, so read that.
...
void solve() {
    ...
    ll ans = 0;
    for (int i=n; i>=1; i--){
        if (i % 2 == 1) ans = max(ans, a[i] + suff[i+1]);
        else ans = max(ans, suff[i+1]);
    }
