...
void dijkstra(vll& cnt, vll& dist, int curNode, const vvpii& adjList){
    int n = sz(adjList);
    vvi parents(n);
    ...
    while (!pq.empty()){
        ...
        for (auto [v, w] : adjList[u]){
            ...
            if (nd < dist[v]){
                dist[v] = nd;
                clr(parents[v]);
                parents[v].pb(u); // new best parent
                pq.push({dist[v], v});
            } 
            else if (nd == dist[v]){
                // another parent on an equal-length shortest path
                parents[v].pb(u);
            }
        }
    }
    ...
    while (!st.empty()){
        ...
        for (int par : parents[cur]) {
            if (!vis[par]) st.push(par);
        }
    }
}

void dijkstra2(vll& dist, int curNode, const vvpii& adjList){
    pq<pii, vpii, greater<>> pq;
    dist[curNode] = 0;
    pq.push({0, curNode});
    ...
    while (!pq.empty()){
        ...
        for (auto [nextNode, w] : adjList[curNode]){
            if (dist[curNode] + w < dist[nextNode]){
...
void solve() {
    ...
    for (int x : a){
        vll dist(n+1, MAX);
        dijkstra(cnt, dist, x, adjList);
    }

    vll dist(n+1, MAX);
    dijkstra2(dist, 1, adjList);
    ll res = ans;
    for (int node=1; node<=n; node++){
        if (cnt[node] >= 1){
            ll temp = ans;
            temp -= dist[node] * cnt[node];
            temp += g * cnt[node];
            res = min(res, temp);
        }
    }
