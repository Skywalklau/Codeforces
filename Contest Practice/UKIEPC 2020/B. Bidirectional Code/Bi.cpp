...
vll brute_force(ll n){
    vll pals;
    for (int i=1; i<=n; i++) if (is_pal(i)) pals.pb(i);

    for (int a : pals) if (a == n) return {a};
...
void solve() {
    ...
    n = stoll(s);
    if (n <= 200000){
        auto small = brute_force(n);
        result.assign(all(small));
        cout << sz(result) << '\n';
    ...
    while (sz(s) > 2){
        int st = sz(s) % 2 == 1 ? sz(s)/2 : sz(s)/2-1;
        str ans = "";
        int j = -1;
        for (int i=st; i>=0; i--){
            if (s[i] != '0'){
        ...
        if (ans == "0"){
            for (int i=0; i<sz(s)-1; i++) ans += '9';
        } 
        else{
            for (int i=j; i<st; i++) ans += '9';
            int n = sz(ans);
            if (sz(s) % 2 == 1){
                for (int i=n-2; i>=0; i--) ans += ans[i];
            } 
            else{
                for (int i=n-1; i>=0; i--) ans += ans[i];
            }
        }
    ...
    if (sz(s) >= 2){
        if (s == "10"){
            ...
            if (s[0] == s[1]) result.pb(stoll(s));
                ...
                result.pb(stoll(x));
                result.pb(stoll(s) - stoll(x));
            }
        }
    } 
    else result.pb(stoll(s));
