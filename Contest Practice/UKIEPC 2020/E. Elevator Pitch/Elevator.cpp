...
void solve() {
    ...
    int ans = 0;
    vvi visited(n+1, vi(m+1));
    for (int i=0; i<n; i++){
        for (int j=0; j<m; j++){
            if (a[i][j] >= 2 && !visited[i][j]){ // bfs to find root   
                ans++; 
                queue<pii> q;
                q.push({i, j});
                set<pii> vis;
                vis.insert({i, j});
                int mx = -1, startX, startY;
                while (!q.empty()){
                    auto [x,y] = q.ft();
                    q.pop();
                    if (a[x][y] > mx){
                    ...
                    for (auto [t1, t2] : dir){
                        ...
                        if (visited[nx][ny] || vis.count({nx,ny})) continue; 
                        if (a[nx][ny] >= a[x][y]){
                            q.push({nx, ny});
                            vis.insert({nx,ny});
                        }
