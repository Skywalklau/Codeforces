...
// Learned a new trick.
// f(f+1)/2, which is the sum of 1 + 2 + 3 + ... + f repeats itself every 2n
// when mod by n.
// Yea its quite obvious what they want, but I just didn't know how to do it
// optimally.
// the eqn is just (x + f(f+1)/2) % n = 0.
// we want to find f.
// But how? The answer is to look at the cycle of f(f+1)/2 % n.
// notice that if f = 2n, then f(f+1)/2 % n = 2n(2n+1)/2 % n = 0.
// then, at f = 4n, (x + f(f+1)/2) % n == x % n as well.
// also, at f = 6n, (x + f(f+1)/2) % n == x % n as well.
// Its a cycle every 2n. So we just need to loop until at most 2n.
