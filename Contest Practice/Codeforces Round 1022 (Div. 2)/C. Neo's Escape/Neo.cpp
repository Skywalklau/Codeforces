...
// solve time ~ 45 mins
// fun problem, used DSU to solve it
//
...
void solve() {
    ...
    for (auto &[val, b] : memo){
        for (int i=0; i<sz(b)-1; i++){
            if (b[i]+1 == b[i+1]) unite(b[i], b[i+1]);
        }
        for (int i=0; i<sz(b); i++){
            if (s.count(b[i]-1) || s.count(b[i]+1)) visited[find(b[i])] = 1;
        }
        
        for (int i=0; i<sz(b); i++){
            if (!visited[find(b[i])]) {ans++; visited[find(b[i])] = 1;}
            s.insert(b[i]);
        }
