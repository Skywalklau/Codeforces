...
// solve time ~ 10 mins
// Trivial problem, just cheese it.
// We need one simple observation, we need 2 distinct number to have
// a cnt of at least 2, then its always possible.
// Because then I can just set the first distinct number with 1 and 2.
// for b[i].
// And the other with 1 and 3 for b[i].
// The rest is all just 1.
// Then this will work.
...
void solve() {
    ...
    map<int, vi> memo;
    for (int i=0; i<n; i++) memo[a[i]].pb(i);

    int found1 = false, found2 = false; 
    vi result(n);
    for (auto [val, c] : memo){
        if (sz(c) <= 1){
            for (int i : c) result[i] = 1;
        }
        else{
            if (!found1){       
                for (int i : c) result[i] = 1;
    ...
    if (found1 && found2) printVector(result);
    else cout << -1 << '\n';
