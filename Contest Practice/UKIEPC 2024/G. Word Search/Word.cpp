...
void check(int x, int y, vvi& visited, vvch& result, int n, int m, vstr& a){
...
void solve() {
    ...
    vvi visited(n2, vi(m2));
    vvch result(n2, vch(m2, '.'));
    for (int i=0; i<n2; i++){
        for (int j=0; j<m2; j++){
            if (b[i][j] == a[0][0]){
                int cnt = 0;
                for (int ii=0; ii<n1; ii++){
                    for (int jj=0; jj<m1; jj++){
                        if (ii+i < n2 && jj+j < m2 && b[ii+i][jj+j] == a[ii][jj]){
                            cnt++;
                            // cout << cnt << endl;
                        } 
                        else {
