...
// This was such a good problem.
// For explanation, pls just read coding sketchbook pg 617 and 618.

pair<bool, pair<ld, ld>> check(ld t, ld A, ld B, ld r){
    ld L = -t, R = t;        // |s| <= t
    L = max(L, -t - r); R = min(R,  t - r);    // |n| <= t
    L = max(L, A - t); R = min(R, A + t);      // |e| <= t
    L = max(L, B - t); R = min(R, B + t);      // |w| <= t
    return {L <= R + 1e-15, mp(L, R)};  // feasible if intervals overlap
}
...
void solve() {
    ...
    ld low = 0, high = 4.0, ans = high;
    ld eps = 1e-7;
    while (high - low >= eps){
        ld mid = (low + high) / 2.0;
        if (check(mid, A, B, r).F){
            ans = mid;
            high = mid;
        }
        else low = mid;
    ...
    // note: (L,R) is an entire valid range, I just took L because we just need a value
    ld res = check(ans, A, B, r).S.F;
    ld s = res;
    ld n = r + s;
    ld e = A - s;
    ld w = B - s;
