...
void solve() {
    ...
    for (int i=1; i<=m; i++){
        ...
        else memo[result[i]] = i;
        cnt[result[i]]++;
    }

    if (!ok) {cout << "impossible" << '\n'; return;}

    pq<array<int, 3>, vector<array<int, 3>>, greater<>> q;
    for (int color=1; color<=k; color++){
        // cout << cnt[color] << " " << memo[color] << " "  << color << '\n';
        if (cnt[color]) q.push({cnt[color], memo[color], color});
        else q.push({0, -MAX, color});
    }
    ...
    while (m != n){
        ...
        while (!found && !q.empty()){
            auto [cnt, idx, color] = q.top();
            q.pop();
            if (m+1 - idx >= p){
                result[m+1] = color;
                found = true;
                q.push({cnt+1, m+1, color});
            }
            else temp.pb({cnt, idx, color});
        }
