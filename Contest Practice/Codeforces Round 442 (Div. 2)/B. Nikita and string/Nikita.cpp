...
void solve() {
    ...
    vi prefA(n+2), suffA(n+2), prefB(n+2);
    for (int i=1; i<=n; i++) prefA[i] = prefA[i-1] + (s[i-1] == 'a');
    for (int i=1; i<=n; i++) prefB[i] = prefB[i-1] + (s[i-1] == 'b');
    for (int i=n; i>=1; i--) suffA[i] = suffA[i+1] + (s[i-1] == 'a');
    ...
    for (int i=0; i<=n; i++){
        for (int j=n+1; j>i; j--){
            int leftA = prefA[i];
            int rightA = suffA[j];
            int midB = prefB[j-1] - prefB[i];
            ans = max(ans, leftA + rightA + midB);
        }
    }
