...
void solve() {
    ...
    int start, end;
    for (int i=0; i<n+m; i++){
        if (c[i].S == 2) {start = i; break;}
    }

    for (int i=n+m-1; i>=0; i--){
        if (c[i].S == 2) {end = i; break;}
    }

    // left to start
    int ans = -MAX;
    ans = max(ans, abs(c.ft().F - c[start].F));

    // right to end
    ans = max(ans, abs(c.bk().F - c[end].F));

    // start to end
    for (int i=start; i<end; i++){
        int j=i+1;
        while (j <= end && c[j].S != 2) j++;
        ...
        // check cities between i and j
        for (int k=i+1; k<j; k++){
            if (c[k].S == 1){
                int d = min(c[k].F - left, right - c[k].F);
                ans = max(ans, d);
            }
        }
