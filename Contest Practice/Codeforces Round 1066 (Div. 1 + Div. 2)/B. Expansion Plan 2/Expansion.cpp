...
void solve() {
    ...
    int nx = sz(s);
    int ny = sz(s);
    x = abs(x), y = abs(y); // make all positive since symmetric anyways
    if (x > nx || y > ny) {cout << "NO" << '\n'; return;}
    int height = nx - x + cnt; // formula from y=0, each 8 increase height of furthest x by 1
    if (height >= y) {cout << "YES" << '\n'; return;}
    cout << "NO" << '\n';
