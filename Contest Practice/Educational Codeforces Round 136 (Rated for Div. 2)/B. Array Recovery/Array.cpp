...
// solve time ~ 8 mins
// Yea this is easy.
// Since d[i] = |a[i] - a[i-1]| then a[i] = +-d[i] + a[i-1]
// Then we have 2 options, op1 and op2 where:
// op1 = d[i] + a[i-1], op2 = -d[i] + a[i-1].
// Since they want non-negative integers a[i], then this means
// the only case we need to check if both op1>=0 && op2>=0 and op1!=op2.
// In that case we know there are 2 ways from this point to construct the
// solution, so we just print -1.
// Otherwise, its always possible, we just take the max (because one 
// of the options is bound to be <0).
