...
void solve() {
    ...
    for (int i=19; i>=1; i--){
        powr[i] = binpow(3, i+1) + i * binpow(3, i-1);
    }
    powr[0] = binpow(3, 1);
    ...
    for (int i=19; i>=1; i--){
        ll cost = powr[i];
        ll cnt = binpow(3, i);
        ll x = y / cnt; // deals made using this deal.
        dealsMade += y/cnt;
        freq[i] = y/cnt;
        ans += (y / cnt) * cost;
        y %= cnt;
    ...
    // cout << dealsMade << endl;
    // cout << ans << endl;
    if (dealsMade > k) cout << -1 << '\n';
    else{
        for (int i=19; i>=1; i--){
            if (freq[i] > 0){
                // cout << i << " " << freq[i] <<  endl;
                // we can fully convert to cheaper deal
                if (3ll * freq[i] + dealsMade - freq[i] <= k){
                    ll cost = powr[i];
                    // cout << cost << endl;
                    ans -= freq[i] * cost;
                    dealsMade -= freq[i];
                    ...
                    ans += 3ll * freq[i] * newCost;
                    dealsMade += 3ll * freq[i];
                    freq[i-1] += 3ll * freq[i];
                }
                else{ // we cannot take all, take remainder
                    ll cost = powr[i];
                    ll remain = k - dealsMade;

                    // how many splits we can afford (each gives +2 slots)
                    ll extraLeft = min(freq[i], remain / 2);
                    ...
                    ans += 3ll * extraLeft * newCost;
                    dealsMade += 3ll * extraLeft;
                    break;
                }
            }
        }
