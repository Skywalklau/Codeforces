...
// For explanation, the editorial is perfect.

void solve() {
	ll n, k, m, x, a;
    cin >> k >> x >> a;

    // bet 1 initially because greedily thats the best move
    ull totalCoins = 1;
    for (int i=1; i<=x; i++){
        // we cannot take ceil, because it could mean minCoin*k >= totalLoss
        // but we want > totalLoss.
        // So we take floor division + 1.
        // minCoinNeededToWinNextBet = totalCoins/(k-1)+1;
        // cout << "loss so far: " << totalCoins << endl;
        // cout << "min coins needed: " << totalCoins/(k-1)+1 << endl;
        totalCoins += totalCoins/(k-1)+1;
    }
    
    cout << (totalCoins <= a ? "YES" : "NO") << '\n';
