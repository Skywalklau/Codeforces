...
// Tried, thought it was monotonic stack, realized it won't work, spend over 1hr on this.
// This was a good problem, learned something new.
// As for the explanation, the editorial is perfect, please read it. 
// Hint: Try think to think from back to front if front to back cannot work.
// Really good problem.
...
//         vpii temp;
//         int curVal = a[i];
//         while (!st.empty() && curVal < st.top().F){
//             int val = st.top().F, cnt = st.top().S;
//             int t1 = val/2, t2 = (val+1)/2;
//             if (!temp.empty() && temp.bk().F == t2) temp.bk().S++;
//             else temp.pb({t2, 1});
//             if (!temp.empty() && temp.bk().F == t1) temp.bk().S++;
//             else temp.pb({t1, 1});
//         }
//         for (int i=sz(temp)-1; i>=0; i--) st.push(temp[i]);
//         st.push({a[i], 1});
//     }
