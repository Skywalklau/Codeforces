...
// in fact, gcd(x,y,z) = gcd(x, y-x, z-x).
// So, gcd(a1, a2, a3, ... , an) = gcd(a1, d)
// where d = gcd(a2​−a1​, a3​−a1​, ..., an​−a1​).
// so, gcd(x+k, y+k, z+k) = gcd(x+k, d)
// where d = gcd(y+k-(x+k), z+k-(x+k)) == gcd(y-x, z-x).
// Notice how the k cancels out.
// Hence, 
// gcd(a1+k, a2+k, a3+k, ..., an+k) == gcd(a1+k, d)
// where d = gcd(a2-a1, a3-a1, a4-a1, ..., an-a1).
// We can just precompute d. 
// then, for every k == b[i], we just do gcd(a1 + k, d), and thats the
// answer.
