...
// solve time ~ can't solve. Beautiful problem.
// As for the explanation, editorial is perfect so read it pls.
...
void solve() {
    ...
    vpll s(n);
    for(int i=0;i<n;i++) cin >> s[i].F;
    for(int i=0;i<n;i++){
        cin >> s[i].S;
        if(s[i].F > s[i].S) swap(s[i].F,s[i].S);
    }

    ll sum = 0;
    for(auto [a,b] : s) sum += abs(a-b);
    sort(all(s));
    ll ans = MAXL;
    for(int i=0;i<n-1;i++){
        ans = min(ans,(s[i+1].F-s[i].S)*2);
    }
...
//     for (int i=0; i<n; i++) a[i][0] = a1[i];
//     for (int i=0; i<n; i++) {a[i][1] = b1[i]; a[i][2] = i;}
...
//         temp.pb(a[i+1][1]);
//         sort(all(temp));
//         ll sum = temp[3] - temp[0] + temp[2] - temp[1];
//         if (sum < ans){
//             ans = sum;
//             idx1 = a[i][2];
//             idx2 = a[i+1][2];
...
//         sort(all(temp));
//         // printVector(temp);
//         sum = temp[3] - temp[0] + temp[2] - temp[1];
//         if (sum < ans){
//             ans = sum;
//             idx1 = b[i][2];
//             idx2 = b[i+1][2];
